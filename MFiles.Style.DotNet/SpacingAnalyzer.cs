...
namespace MFiles.Style.DotNet
{
	...
	public class SpacingAnalyzer : DiagnosticAnalyzer
	{
        ...
        public static readonly RuleDescription SpacesWithinBrackets = new RuleDescription( "SpacesWithinBrackets", "Spacing" );
        ...
        private static void CheckBracket( SyntaxNodeAnalysisContext context, SyntaxToken bracket )
        {
            ...
            var text = bracket.GetLocation().SourceTree.GetText().GetSubText( TextSpan.FromBounds( spanStart, spanStart + 1 ) ).ToString();
            if( text.Trim() == "" )
                return;
